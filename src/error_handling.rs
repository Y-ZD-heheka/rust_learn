    ...
    impl fmt::Display for NetworkError {
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
            match self {
                ...
                Self::BadRequest { status, message } => write!(f, "客户端错误 {}: {}", status, message),
                Self::ServerError { status, message } => write!(f, "服务器错误 {}: {}", status, message),
        ...
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
            match self {
                ...
                Self::InvalidType { field, expected, actual } => {
                    write!(f, "字段 {} 类型错误，期望 {}，实际 {}", field, expected, actual)
                },
        ...
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
            match self {
                ...
                Self::ValueOutOfRange { field, min, max, actual } => {
                    write!(f, "字段 {} 值超出范围 [{}, {}]，实际: {}", field, min, max, actual)
                },
...
pub fn resource_loading_error_handling() {
    ...
    impl ResourceLoader {
        ...
        fn load_image(&mut self, path: &str) -> Result<Vec<u8>, ResourceError> {
            ...
            match path {
                ...
                "slow_network.webp" => return Err(ResourceError::NetworkError("网络延迟".to_string())),
                _ => {
                    // 模拟成功加载
        ...
        fn load_font(&mut self, font_name: &str) -> Result<Vec<u8>, ResourceError> {
            match font_name {
                "admin_font.ttf" => return Err(ResourceError::PermissionDenied("管理员字体".to_string())),
    ...
    // 测试图片加载
    let images = vec!["avatar.png", "missing.png", "corrupted.jpg", "slow_network.webp"];
...
pub fn external_service_error_handling() {
    ...
    impl fmt::Display for ServiceError {
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
            match self {
                Self::RateLimitExceeded { service, retry_after } => {
