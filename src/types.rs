...
pub fn advanced_trait_objects() {
    ...
    let shapes: Vec<Box<dyn SimpleShape>> = vec![
        ...
        Box::new(SimpleRectangle { width: 4.0, height: 6.0 }),
    ...
    for (i, shape) in shapes.iter().enumerate() {
        println!("形状{}: {} (面积: {:.2})", i + 1, shape.name(), shape.area());
    }
}
...
pub fn associated_types() {
    ...
    let beijing = City { name: "北京".to_string(), population: 21540000 };
    let shanghai = City { name: "上海".to_string(), population: 24870000 };
    let guangzhou = City { name: "广州".to_string(), population: 15300000 };
    
    graph.add_node(beijing.clone());
    graph.add_node(shanghai.clone());
    graph.add_node(guangzhou.clone());
    
    let beijing_to_shanghai = Route { distance: 1200.0, travel_time: 8.5 };
...
pub fn composition_patterns() {
    ...
    branch1.add(Box::new(Leaf { name: "叶子1".to_string() }));
    branch1.add(Box::new(Leaf { name: "叶子2".to_string() }));
    
    branch2.add(Box::new(Leaf { name: "叶子3".to_string() }));
    ...
    let simple = SimpleDataSource { data: "原始数据".to_string() };
