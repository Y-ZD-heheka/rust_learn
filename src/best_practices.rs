...
pub fn modern_error_handling_best_practices() {
    ...
    fn validate_user_input(name: &str, age: u32) -> Result<String, BusinessError> {
        if name.trim().is_empty() {
            return Err(BusinessError::ValidationFailed("用户名不能为空".to_string()));
        }
        
        if age < 13 {
            return Err(BusinessError::ValidationFailed("用户年龄必须大于等于13岁".to_string()));
        }
...
pub fn testing_best_practices() {
    ...
    for items in int_cases {
        ...
        println!("  整数列表 {:?}: {}", items, if result { "✅" } else { "❌" });
    }
